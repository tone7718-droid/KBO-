...
| `npm run parse:netlog -- <NetLog.json>` | Chrome 내장 `chrome://net-export/` 덤프를 오프라인 파싱 (자동화 0, 가장 강한 우회) |
...
### scheduleId 수집 — Chrome 내장 NetLog 캡처 (최신, 권장)

PerimeterX(봇 차단)는 자동화된 fetch/XHR을 모두 막고, F12(DevTools)도 차단합니다. 그러나 Chrome은 **DevTools와 완전히 분리된** 네트워크 덤프 기능을 내장하고 있습니다: `chrome://net-export/`. 이를 켜고 사용자가 평범하게 ticketlink을 한 번 방문하면, schedules API의 응답 본문까지 디스크에 저장됩니다. 우리는 그 파일을 오프라인에서 파싱하기만 하면 됩니다 — **브라우저나 페이지에 우리 코드가 한 줄도 들어가지 않습니다.**

왜 통과하는가:
- `chrome://net-export/` 는 DevTools가 아닌 별개 페이지 — PerimeterX의 F12 차단이 무효
- 자동화 시그니처(navigator.webdriver, --enable-automation, CDP attach 등)가 0
- 단순한 사람-브라우징이라 봇 탐지 트리거 없음

절차 (PowerShell 한 창만 있으면 됨):

```powershell
# 1) Chrome 주소창에 입력: chrome://net-export/
#    (사용 중인 Chrome 어느 창에서든 OK. attach 모드용 디버그 포트 Chrome일 필요 없음)

# 2) [중요] "Include raw bytes (includes cookies and credentials, ...)" 체크박스를 켭니다.
#    이 옵션이 꺼져있으면 응답 본문이 저장되지 않습니다.

# 3) "Start Logging to Disk" 클릭 → 저장 경로 선택 (예: C:\temp\ticketlink.json)

# 4) 새 탭 또는 기존 탭에서 https://m.ticketlink.co.kr/sports/137/57 방문/새로고침
#    페이지가 정상 로드되어 경기 카드가 보일 때까지 대기 (3~5초 정도)
#    필요하면 월/날짜 필터를 클릭해서 더 많은 데이터를 트리거해도 좋음

# 5) chrome://net-export/ 탭으로 돌아가 "Stop Logging" 클릭
...
성공하면 `booking-data.json`이 갱신됩니다 (`mode: "netlog"`, `strategy: "chrome-net-export-offline-parse"`). 실패 시 친절한 한국어 진단을 출력합니다:
- `body empty in netlog — "Include raw bytes" 옵션 미체크?` → 2단계 체크박스 잊음
- `schedules API 호출이 NetLog에 없습니다` → 4단계에서 ticketlink 페이지를 방문하지 않음, 또는 PerimeterX가 차단 — Chrome 창에서 페이지가 정상 표시되는지 다시 확인
- `JSON parse 실패 (꼬리 잘린 NetLog일 가능성)` → 자동 복구 시도. 그래도 실패하면 5단계 "Stop Logging" 클릭 누락

NetLog 파일은 본인의 쿠키/세션을 포함하므로 git에 커밋하지 말고 로컬에만 두세요.

### scheduleId 수집 — API 직접 호출 (attach 모드, 폴백)
...
├── scripts/parse-netlog.js     # chrome://net-export 덤프 오프라인 파서 (PerimeterX 우회용)
├── scripts/parse-local-html.js # 브라우저에서 저장한 HTML 오프라인 파서
├── scripts/discover.js         # attach 모드 DOM/네트워크 진단 덤프
